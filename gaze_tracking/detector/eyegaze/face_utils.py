...
if os.path.exists('./detector/eyegaze/bfm_slim.pkl'):
    bfm = _load('./detector/eyegaze/bfm_slim.pkl')
    r = _load('./detector/eyegaze/param_mean_std_62d_120x120.pkl')
else:
    bfm = _load('./gaze_tracking/detector/eyegaze/bfm_slim.pkl')
    r = _load('./gaze_tracking/detector/eyegaze/param_mean_std_62d_120x120.pkl')
...
keypoints = bfm.get('keypoints').astype(np.int64)  # fix bug
...
def TDDFA_detector(session, face_box, sx, sy, ex, ey):
    ...
    return head_vers.T[:,:2], head_vers

def headgaze_detector(head_vers):
    # Calc head gaze vector
...
def get_left_eye_roi(image, vers):
    eye_ROI_w = dist_2D(np.array([vers[42,0], vers[42,1]]), np.array([vers[45,0], vers[45,1]]))
    
    center_x = (vers[42,0] + vers[45,0]) / 2.0
    half_w = (eye_ROI_w / 2.0) + (eye_ROI_w * 0.4) # Add 40% margin to each side to prevent cut off
    
    eye_cy = int(round((vers[44,1] + vers[46,1])/2))
    eye_sy = int(eye_cy - 40)
    eye_ey = int(eye_cy + 40)
    
    eye_sx = int(center_x - half_w)
    eye_ex = int(center_x + half_w)
    
    eye_sx = max(eye_sx, 0)
    eye_sy = max(eye_sy, 0)
    eye_ex = min(eye_ex, image.shape[1])
    eye_ey = min(eye_ey, image.shape[0])
    
    eye_img = image[eye_sy:eye_ey, eye_sx:eye_ex].copy()
    return eye_img, [eye_sx, eye_sy, vers[42,0], vers[42,1]]

def get_right_eye_roi(image, vers):
    eye_ROI_w = dist_2D(np.array([vers[36,0], vers[36,1]]), np.array([vers[39,0], vers[39,1]]))
    
    center_x = (vers[36,0] + vers[39,0]) / 2.0
    half_w = (eye_ROI_w / 2.0) + (eye_ROI_w * 0.4) # Add 40% margin to each side to prevent cut off
    ...
    eye_img = image[eye_sy:eye_ey, eye_sx:eye_ex].copy()
    return eye_img, [eye_sx, eye_sy, vers[36,0], vers[36,1]]
